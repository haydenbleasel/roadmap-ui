---
title: Calendar
description: The calendar view displays features on a grid calendar. Specifically it shows the end date of each feature, and groups features by day.
icon: CalendarRange
---

import { AutoTypeTable } from 'fumadocs-typescript/ui';

<div className="not-prose w-full bg-white border rounded-xl overflow-hidden">
  <Calendar />
</div>

## Installation

```sh title="Terminal"
npx shadcn@latest add https://roadmap-ui.com/registry/calendar.json
```

## Features

- Features are grouped by day
- Features are color-coded by their status
- Features are truncated if there are too many to fit in the grid cell

## Subcomponents

The Calendar component is made up of the following subcomponents:

### CalendarProvider

The `CalendarProvider` component is used to provide the features to the calendar.

<AutoTypeTable path="registry/block/calendar.tsx" name="CalendarProviderProps" />

### CalendarDate

The `CalendarDate` component is used to display the date of the calendar.

<AutoTypeTable path="registry/block/calendar.tsx" name="CalendarDateProps" />

### CalendarDateLabel

The `CalendarDateLabel` component is used to display the date of the calendar.

<AutoTypeTable path="registry/block/calendar.tsx" name="CalendarDateLabelProps" />

### CalendarDatePagination

The `CalendarDatePagination` component is used to display the pagination of the calendar.

<AutoTypeTable path="registry/block/calendar.tsx" name="CalendarDatePaginationProps" />

### CalendarHeader

The `CalendarHeader` component is used to display the header of the calendar.

<AutoTypeTable path="registry/block/calendar.tsx" name="CalendarHeaderProps" />

### CalendarBody

The `CalendarBody` component is used to display the body of the calendar.

<AutoTypeTable path="registry/block/calendar.tsx" name="CalendarBodyProps" />

### CalendarItem

The `CalendarItem` component is used to display a single item in the calendar.

<AutoTypeTable path="registry/block/calendar.tsx" name="CalendarItemProps" />
